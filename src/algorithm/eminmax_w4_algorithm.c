                ...
                const VECTOR *restrict r, size_t num_anchors, int width __attribute__((__unused__)), int height __attribute__((__unused__)), VECTOR *restrict resx, VECTOR *restrict resy)
{
    ...
    for (int j = 0; j < 4; j++) {
        ...
        for (size_t i = 0; i < num_anchors; i++) {		
            weights[j] += VECTOR_ABS(distance(corners_x[j],corners_y[j],vx[i],vy[i]) * distance(corners_x[j],corners_y[j],vx[i],vy[i]) - r[i]*r[i]);
        }
        weights[j] = one / VECTOR_MAX(weights[j], VECTOR_BROADCASTF(FLT_EPSILON));
    }
