...
static ls2_nd_noise_arguments nd_noise_arguments;



static GOptionEntry nd_noise_parameters[] = {
        { "nd-mean", 0, 0, G_OPTION_ARG_DOUBLE, &nd_noise_arguments.mean,
          "mean value of the error", NULL },
        { "nd-sdev", 0, 0, G_OPTION_ARG_DOUBLE, &nd_noise_arguments.sdev,
...
ls2_add_nd_noise_option_group(GOptionContext *context)
{
     ...
     group = g_option_group_new("nd-noise",
                                ...
                                "Parameters to the Gaussian noise error model",
                                NULL, NULL);
     g_option_group_add_entries(group, nd_noise_parameters);
               ...
               VECTOR *restrict result)
{
        for (size_t k=0; k < anchors ; k++) {
                VECTOR rn = gaussrand(seed, (float) nd_noise_arguments.mean,
                                      (float) nd_noise_arguments.sdev);
